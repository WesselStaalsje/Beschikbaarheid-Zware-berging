...
export function AdminDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#eef1f4] text-[#17212b]">
      <header className="border-b border-[#24313d] bg-[#101820] text-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6"><div className="flex items-center gap-3"><UserRoundCog className="text-[#f9b233]" /><div><h1 className="font-bold">Beheer Plusdiensten</h1><p className="text-xs text-white/55">Vestigingen, chauffeurs en achterwacht</p></div></div><div className="flex gap-2"><Link href="/" className="flex items-center gap-2 border border-white/20 px-3 py-2 text-xs font-bold"><ArrowLeft className="size-4" /> Dashboard</Link><button onClick={async () => { await fetch("/api/admin/session", { method: "DELETE" }); setAuthenticated(false); }} className="border border-white/20 p-2" aria-label="Uitloggen"><LogOut className="size-4" /></button></div></div></header>
      <div className="mx-auto grid max-w-6xl gap-5 px-4 py-6 sm:px-6 lg:grid-cols-2">
        ...
        <section className="bg-white shadow-sm lg:col-span-2">
          <div className="flex flex-col gap-3 border-b px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><CalendarDays className="size-5 text-[#f9b233]"/><div><h2 className="font-bold">Achterwacht</h2><p className="text-xs text-slate-500">Wijzig hier een ruil; het dashboard neemt dit direct over.</p></div></div><input type="month" value={standbyMonth} onChange={(e) => setStandbyMonth(e.target.value)} className="border px-3 py-2 text-sm" aria-label="Maand achterwacht" /></div>
