...
type Responder = { id: string; name: string; depotId: string; sortOrder: number; active: boolean };
    ...
    <main className="min-h-screen bg-[#eef1f4] text-[#17212b]">
      ...
      <div className="mx-auto grid max-w-6xl gap-5 px-4 py-6 sm:px-6 lg:grid-cols-2">
        ...
        <section className="bg-white shadow-sm"><div className="flex items-center gap-2 border-b px-5 py-4"><Truck className="size-5 text-slate-500"/><h2 className="font-bold">Chauffeurs</h2></div><div className="divide-y">
          {responders.map((responder) => <ResponderRow key={responder.id} responder={responder} depots={depots} busy={busy} save={(name, depotId, active) => void action({ action: "update-responder", id: responder.id, name, depotId, active }, "Chauffeur opgeslagen")} />)}
...
function ResponderRow({ responder, depots, busy, save }: { responder: Responder; depots: Depot[]; busy: boolean; save: (name: string, depotId: string, active: boolean) => void }) {
  const [name, setName] = useState(responder.name); const [depotId, setDepotId] = useState(responder.depotId); const [active, setActive] = useState(responder.active);
  return <div className="grid gap-2 p-4 sm:grid-cols-[1fr_1fr_auto_auto] sm:items-center"><input value={name} onChange={(e) => setName(e.target.value)} className="min-w-0 border px-3 py-2 text-sm"/><select value={depotId} onChange={(e) => setDepotId(e.target.value)} className="border px-3 py-2 text-sm">{depots.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select><label className="flex items-center gap-2 text-xs font-semibold"><input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)}/> Actief</label><button disabled={busy || !name.trim()} onClick={() => save(name, depotId, active)} className="p-2 text-slate-500 hover:bg-slate-100" aria-label={`${responder.name} opslaan`}><Save className="size-4"/></button></div>;
}
