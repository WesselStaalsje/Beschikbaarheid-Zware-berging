...
export async function GET() {
  try {
    ...
    const [depots, responders] = await Promise.all([
      ...
      sql`SELECT r.id, r.name, d.name AS depot, r.status, r.updated_at AS "updatedAt", r.updated_by AS "updatedBy"
...
export async function PUT(request: Request) {
  try {
    ...
    const rows = (await sql`
      UPDATE responders r SET status = ${input.status}, updated_at = ${updatedAt}, updated_by = ${updatedBy}
      FROM depots d WHERE r.id = ${input.id} AND r.active = TRUE AND d.id = r.depot_id AND d.active = TRUE
      RETURNING r.id, r.name, d.name AS depot, r.status, r.updated_at AS "updatedAt", r.updated_by AS "updatedBy"
    `) as Array<{ id: string; name: string; depot: string; status: string; updatedAt: string; updatedBy: string }>;
